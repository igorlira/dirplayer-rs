...
const CONFIG: &str = include_str!("../configs/habbo_v7.toml");
...
browser_e2e_test!(test_habbo_v7_login, |player| async move {
    ...
    cfg.apply_external_params();
    shared::assert_entry(&mut player, cfg.suite(), &cfg.movie.path).await?;
    shared::assert_login(&mut player, cfg.suite(), cfg.param("username"), cfg.param("password")).await?;
    shared::assert_navigate_pub(&mut player, cfg.suite()).await?;
    Ok(())
});
