...
const CONFIG: &str = include_str!("../configs/intel_dummy.toml");

browser_e2e_test!(test_intel_dummy_load, |player| async move {
    let cfg = TestConfig::from_toml(CONFIG);
    cfg.apply_external_params();
    let movie_path = player.asset_path(&cfg.movie.path);
    let snapshots = SnapshotContext::new(cfg.suite(), "dummy");
    ...
    snapshots.verify("perspective_01", player.snapshot_stage())?;

    player.click_sprite(sprite().number(5)).await?;

    player.step_frames(200).await;

    snapshots.verify("perspective_02", player.snapshot_stage())?;
