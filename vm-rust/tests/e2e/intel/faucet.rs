...
const CONFIG: &str = include_str!("../configs/intel_faucet.toml");

browser_e2e_test!(test_intel_faucet_load, |player| async move {
    let cfg = TestConfig::from_toml(CONFIG);
    cfg.apply_external_params();
    let movie_path = player.asset_path(&cfg.movie.path);
    let snapshots = SnapshotContext::new(cfg.suite(), "faucet");
