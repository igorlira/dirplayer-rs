...
const CONFIG: &str = include_str!("../configs/lego_roboriders_dust.toml");

browser_e2e_test!(test_roboriders_dust_load, |player| async move {
    let cfg = TestConfig::from_toml(CONFIG);
    cfg.apply_external_params();
    let movie_path = player.asset_path(&cfg.movie.path);
    let snapshots = SnapshotContext::new(cfg.suite(), "roboriders_dust");
    ...
    player.click_sprite(sprite().number(69)).await?; // START LEVEL 1

    player.step_until(datum("_movie.frame").equals(StaticDatum::Int(20))).timeout(15.0).await?;

    player.step_until(sprite().member("rider dust main").visible(1.0)).await?;
