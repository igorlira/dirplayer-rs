...
const CONFIG: &str = include_str!("../configs/lego_roboriders_swamp.toml");

browser_e2e_test!(test_roboriders_swamp_load, |player| async move {
    let cfg = TestConfig::from_toml(CONFIG);
    cfg.apply_external_params();
    let movie_path = player.asset_path(&cfg.movie.path);
    let snapshots = SnapshotContext::new(cfg.suite(), "roboriders_swamp");
    ...
    player.step_until(sprite().member("rider swamp main").visible(1.0)).await?;
