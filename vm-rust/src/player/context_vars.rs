  ...
  match var_type {
    ...
    0x6 => {
      ...
      let member_ref_opt = player
        .movie
        .cast_manager
        .find_member_ref_by_identifiers(&player.get_datum(id_ref), cast_id_opt, &player.allocator)
      ...
      if let Some(member) = player.movie.cast_manager.find_mut_member_by_ref(&member_ref) {
        match &mut member.member_type {
          ...
          other => {
            console::log_1(&format!(
              "❌ Member exists but is not a Field: {:?}", other
            ).into());
            Err(ScriptError::new("Cast member exists but is not a Field".to_string()))
          }
        }
      } else {
        console::log_1(&format!(
        ...
        Err(ScriptError::new("Field member not found in cast_manager".to_string()))
      }
    }
