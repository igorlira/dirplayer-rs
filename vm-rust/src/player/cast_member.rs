  ...
  pub fn from(
    ...
    let member_type = match chunk.member_type {
      ...
      MemberType::Font => {
        ...
        let preview_text = member_def.children.iter().find_map(|child_opt| {
          if let Some(Chunk::XMedia(xmedia)) = child_opt {
            if !xmedia.is_pfr_font() && xmedia.raw_data.len() > 100 {
              ...
              let preview: String = xmedia.raw_data.iter()
                  .take(200)
                  .map(|b| format!("{:02X}", b))
                  .collect::<Vec<_>>()
                  .join(" ");
              ...
              for i in 0..xmedia.raw_data.len().saturating_sub(100) {
                if xmedia.raw_data[i] == 0x00 && 
                  ...
                  xmedia.raw_data[i + 3] == 0x2C { // comma
                  ...
                  for j in text_start..xmedia.raw_data.len().min(text_start + 500) {
              ...
              for i in 0..xmedia.raw_data.len().saturating_sub(20) {
                if xmedia.raw_data[i..].starts_with(b"FFF Reaction") {
                  ...
                  if !name.is_empty() {
                    ...
                    console::log_1(&format!("📝 Found font reference: '{}'", name_str).into());
        ...
        let (font_info, bitmap_ref, char_width, char_height, grid_columns, grid_rows) = 
          if let Some(pfr) = pfr_font_data {
            console::log_1(&format!(
              ...
              pfr.font_name, pfr.char_width, pfr.char_height,
              pfr.grid_columns, pfr.grid_rows
            ).into());
                ...
                for col in 0..pfr.char_width {
                  ...
                  if bit_set {
                    ...
                    if px < bitmap_width && py < bitmap_height {
                      let pixel_idx = (py as usize * bitmap_width as usize + px as usize) * 4;
                      if pixel_idx + 3 < font_bitmap.data.len() {
                        ...
                        font_bitmap.data[pixel_idx + 3] = 255; // A
                      }
                    }
                  }
                }
            ...
            (font_info, Some(bitmap_ref), Some(pfr.char_width), Some(pfr.char_height), 
            Some(pfr.grid_columns), Some(pfr.grid_rows))
        ...
        console::log_1(&format!(
          ...
          font_info.name, bitmap_ref.is_some(), preview_text.len()
        ).into());
