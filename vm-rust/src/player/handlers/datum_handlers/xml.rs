...
impl XmlParser {
        ...
        for event in parser {
            match event {
                ...
                Ok(XmlEvent::ProcessingInstruction { name, data }) => {
                    if let Some(&parent_id) = element_stack.last() {
                        self.add_processing_instruction_node(&mut nodes, parent_id, &name, data.as_deref());
                    }
                },
    ...
    fn convert_attributes(attributes: Vec<OwnedAttribute>) -> HashMap<String, String> {
        ...
        for attr in attributes {
            ...
            attr_map.insert(attr.name.local_name.to_lowercase(), attr.value.trim_matches('"').to_string());
        }
        attr_map
    }
    ...
    fn add_text_node(&mut self, nodes: &mut HashMap<u32, XmlNode>, parent_id: u32, text: &str) {
        ...
        web_sys::console::log_1(&format!("🔧 Created text node: '{}' (ID: {})", text, text_id).into());
    }
    ...
    fn add_comment_node(&mut self, nodes: &mut HashMap<u32, XmlNode>, parent_id: u32, comment: &str) {
    ...
    fn add_processing_instruction_node(&mut self, nodes: &mut HashMap<u32, XmlNode>, parent_id: u32, name: &str, data: Option<&str>) {
...
impl XmlHelper {
    ...
    fn search_nodes_recursive(
        ...
        let (name, children): (String, Vec<u32>) = if let Some(node) = player.xml_nodes.get(&node_id) {
...
impl XmlDatumHandlers {
    ...
    fn parse_xml(datum: &DatumRef, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        if args.is_empty() {
            return Err(ScriptError::new("parseXML requires XML string argument".to_string()));
        }
        ...
        reserve_player_mut(|player| {
            ...
            let parser_id = match player.get_datum(datum) {
                Datum::XmlRef(id) => *id,
                _ => {
                    return Err(ScriptError::new("parseXML must be called on XML parser object".to_string()));
                }
            };
            
            web_sys::console::log_1(&format!("🔧 parseXML called on parser object ID {}", parser_id).into());
            ...
            let xml_string = match arg_datum {
                ...
                Datum::Void => {
                    return Err(ScriptError::new("parseXML argument is Void - expected XML string".to_string()));
                }
                _ => {
            ...
            web_sys::console::log_1(&format!("🔧 XML.parseXML called with {} characters", xml_string.len()).into());
            ...
            web_sys::console::log_1(&format!("🔧 Parsing complete. root_id={:?}, parsed {} nodes", root_id, nodes.len()).into());
            ...
            if let Some(parser_doc) = player.xml_documents.get_mut(&parser_id) {
                ...
                web_sys::console::log_1(&format!("🔧 Updated parser object {} with root_element={:?}", parser_id, root_id).into());
            } else {
                ...
                web_sys::console::log_1(&format!("🔧 Created document for parser object {} with root_element={:?}", parser_id, root_id).into());
            }
    ...
    fn create_element(datum: &DatumRef, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        if args.is_empty() {
            return Err(ScriptError::new("createElement requires element name argument".to_string()));
        }
        ...
        reserve_player_mut(|player| {
            ...
            let doc_id = match player.get_datum(datum) {
                Datum::XmlRef(id) => *id,
                _ => {
                    return Err(ScriptError::new("createElement must be called on XML document".to_string()));
                }
            };
            ...
            let element_name = match player.get_datum(&args[0]) {
                ...
                _ => {
                    return Err(ScriptError::new("createElement requires string element name".to_string()));
                }
            };
            
            web_sys::console::log_1(&format!("🔧 createElement('{}') called on doc {}", element_name, doc_id).into());
            ...
            web_sys::console::log_1(&format!("🔧 Created element '{}' with ID {}", element_name, element_id).into());
    ...
    fn append_child(datum: &DatumRef, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        if args.is_empty() {
            return Err(ScriptError::new("appendChild requires child node argument".to_string()));
        }
        ...
        reserve_player_mut(|player| {
            ...
            let parent_id = match player.get_datum(datum) {
                Datum::XmlRef(id) => *id,
                _ => {
                    return Err(ScriptError::new("appendChild must be called on XML node".to_string()));
                }
            };
            ...
            let child_id = match player.get_datum(&args[0]) {
                Datum::XmlRef(id) => *id,
                _ => {
                    return Err(ScriptError::new("appendChild requires XML node argument".to_string()));
                }
            };
            
            web_sys::console::log_1(&format!("🔧 appendChild: parent={}, child={}", parent_id, child_id).into());
            ...
            if let Some(doc) = player.xml_documents.get_mut(&parent_id) {
                ...
                web_sys::console::log_1(&format!("🔧 Set child {} as root of document {}", child_id, parent_id).into());
                ...
                web_sys::console::log_1(&format!("🔧 Added child {} to node {}", child_id, parent_id).into());
            } else {
                return Err(ScriptError::new(format!("Parent node {} not found", parent_id)));
            }
    ...
    fn create_xml_document(player: &mut DirPlayer, content: String) -> Result<u32, ScriptError> {
        ...
        web_sys::console::log_1(&format!("🔧 Storing document {} with root_element={:?}", doc_id, root_id).into());
        player.xml_documents.insert(doc_id, xml_doc);
        
        // Verify storage
        if player.xml_documents.contains_key(&doc_id) {
            web_sys::console::log_1(&format!("🔧 ✓ Document {} stored successfully", doc_id).into());
        } else {
    ...
    fn to_string(datum: &DatumRef, _args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        reserve_player_mut(|player| {
            let xml_id = match player.get_datum(datum) {
                Datum::XmlRef(id) => *id,
                _ => {
                    return Err(ScriptError::new("toString must be called on XML object".to_string()));
                }
            };
    ...
    // Get XML property
    fn get_xml_property(player: &mut DirPlayer, xml_id: u32, prop: &str) -> Result<DatumRef, ScriptError> {
        match prop {
            "firstChild" => {
                // Check if it's a document
                if let Some(doc) = player.xml_documents.get(&xml_id) {
                    if let Some(root_id) = doc.root_element {
                        ...
                        web_sys::console::log_1(&format!("🔧 ⚠️ Document {} has no root_element (None)", xml_id).into());
                    }
                ...
                if let Some(node) = player.xml_nodes.get(&xml_id) {
                    ...
                    if let Some(first_child) = children.first() {
                        ...
                        web_sys::console::log_1(&format!("🔧 Node has no accessible children").into());
                    }
                } else {
                    web_sys::console::log_1(&format!("🔧 ⚠️ xml_id {} is neither a document nor a node!", xml_id).into());
                    web_sys::console::log_1(&format!("🔧 Documents in storage: {:?}", player.xml_documents.keys().collect::<Vec<_>>()).into());
                    web_sys::console::log_1(&format!("🔧 Nodes in storage: {:?}", player.xml_nodes.keys().collect::<Vec<_>>()).into());
                }
            ...
            "ignoreWhite" => {
                let ignore = player.xml_documents
                    .get(&xml_id)
    ...
    fn set_xml_property(player: &mut DirPlayer, xml_id: u32, prop: &str, value: &DatumRef) -> Result<(), ScriptError> {
        match prop {
            "ignoreWhite" => {
            ...
            attr_name if xml_id > 10000 => {
                ...
                web_sys::console::log_1(&format!("🔧 Set attribute '{}' = '{}' on node {}", attr_name, value_str, node_id).into());
            ...
            _ => Err(ScriptError::new(format!("Cannot set XML property: {}", prop))),
        }
    }
