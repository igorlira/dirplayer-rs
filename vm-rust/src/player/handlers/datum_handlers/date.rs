...
impl DateDatumHandlers {
    pub fn call(datum: &DatumRef, handler_name: &String, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        reserve_player_mut(|player| {
            ...
            let date_obj = player.date_objects.get(&date_id)
                .ok_or_else(|| ScriptError::new(format!("Date object {} not found", date_id)))?;
            
            let js_date = js_sys::Date::new(&wasm_bindgen::JsValue::from_f64(date_obj.timestamp_ms as f64));
            ...
            match handler_name.as_str() {
                ...
                "setTime" => {
                    if args.is_empty() {
                        return Err(ScriptError::new("setTime requires a time argument".to_string()));
                    }
                    let time = player.get_datum(&args[0]).int_value()? as i64;
                    let date_obj = player.date_objects.get_mut(&date_id)
                        .ok_or_else(|| ScriptError::new(format!("Date object {} not found", date_id)))?;
                ...
                "setFullYear" => {
                    if args.is_empty() {
                        return Err(ScriptError::new("setFullYear requires a year argument".to_string()));
                    }
                    let year = player.get_datum(&args[0]).int_value()?;
                    let mut js_date = js_sys::Date::new(&wasm_bindgen::JsValue::from_f64(date_obj.timestamp_ms as f64));
                ...
                "setMonth" => {
                    if args.is_empty() {
                        return Err(ScriptError::new("setMonth requires a month argument".to_string()));
                    }
                ...
                "setDate" => {
                    if args.is_empty() {
                        return Err(ScriptError::new("setDate requires a date argument".to_string()));
                    }
                ...
                "setHours" => {
                    if args.is_empty() {
                        return Err(ScriptError::new("setHours requires an hours argument".to_string()));
                    }
                ...
                "setMinutes" => {
                    if args.is_empty() {
                        return Err(ScriptError::new("setMinutes requires a minutes argument".to_string()));
                    }
                ...
                "setSeconds" => {
                    if args.is_empty() {
                        return Err(ScriptError::new("setSeconds requires a seconds argument".to_string()));
                    }
                ...
                _ => Err(ScriptError::new(format!("No handler {} for date", handler_name)))
            }
        })
    }
    ...
    pub fn get_prop(player: &mut DirPlayer, datum: &DatumRef, prop: &String) -> Result<DatumRef, ScriptError> {
        match prop.as_str() {
            ...
            _ => Err(ScriptError::new(format!("Cannot get date property {}", prop)))
        }
    }

    pub fn set_prop(_player: &mut DirPlayer, _datum: &DatumRef, prop: &String, _value: &DatumRef) -> Result<(), ScriptError> {
        Err(ScriptError::new(format!("Cannot set date property {}", prop)))
    }
}