use crate::{console_warn, director::lingo::datum::Datum, player::{reserve_player_mut, reserve_player_ref, timeout::Timeout, DatumRef, DirPlayer, ScriptError}};
...
impl TimeoutDatumHandlers {
  ...
  pub async fn call_async(datum: &DatumRef, handler_name: &String, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
    match handler_name.as_str() {
      ...
      _ => Err(ScriptError::new(format!("No async handler {handler_name} for timeout")))
    }
  }
  ...
  pub async fn new(datum: &DatumRef, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
    let timeout_name = reserve_player_ref(|player| {
      ...
      match timeout_datum {
        ...
        _ => Err(ScriptError::new("Cannot create timeout from non-timeout".to_string())),
      }
    })?;

    // Check if this timeout name corresponds to a script in the cast
    let script_ref = reserve_player_ref(|player| {
      player.movie.cast_manager.find_member_ref_by_name(&timeout_name)
    });
    ...
    let timeout_handler = reserve_player_ref(|player| {
      match player.get_datum(&args[1]) {
        ...
        _ => Err(ScriptError::new("Timeout handler must be a string or symbol".to_string())),
      }
    })?;
