use crate::{
    ...
    player::{DirPlayer, ScriptError, DatumRef},
};

use std::sync::Arc;
use wasm_bindgen::JsCast;
use web_sys::{AudioContext, AudioBuffer, OfflineAudioContext, AudioBufferSourceNode, GainNode, StereoPannerNode, Blob, Url, HtmlAudioElement};
...
const STEP_TABLE: [i32; 89] = [
    ...
    13899, 15289, 16818, 18500, 20350, 22385, 24623, 27086, 29794, 32773
];

const INDEX_TABLE: [i32; 16] = [
    -1, -1, -1, -1, 2, 4, 6, 8, // For the lower nibble
    -1, -1, -1, -1, 2, 4, 6, 8, // (The full nibble 0-15 is used to map, but the first 8 and last 8 are often symmetrical)
];
...
impl SoundChannelDatumHandlers {
    ...
    pub fn play_segment_for_member(
        ...
        let sound_member = {
            ...
            if let Datum::CastMember(ref cast_member_ref) = member_datum {
                if let Some(cast_member) = player.movie.cast_manager.find_member_by_ref(cast_member_ref) {
        ...
        let sound_member = sound_member.ok_or_else(|| ScriptError::new("Invalid sound member".to_string()))?;

        let channel_rc = player.sound_manager.get_channel_mut(channel_idx)
    ...
    pub fn call(player: &mut DirPlayer, datum: &DatumRef, handler_name: &String, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        ...
        match handler_name_lower.as_str() {
            ...
            "playfile" => {
                if args.is_empty() {
                    return Err(ScriptError::new("playFile requires a member argument".to_string()));
                }
            ...
            "queue" => {
                if args.is_empty() {
                    return Err(ScriptError::new("queue requires a member argument".to_string()));
                }
            ...
            "fadein" => {
                let ticks = if args.is_empty() { 60 } else {
            ...
            "fadeto" => {
                if args.len() < 2 {
                    return Err(ScriptError::new("fadeTo requires ticks and volume arguments".to_string()));
                }
            ...
            "setplaylist" => {
                if args.is_empty() {
                    return Err(ScriptError::new("setPlayList requires a list argument".to_string()));
                }
                Self::handle_set_playlist(player, datum, &args[0])?;
                Ok(datum.clone())
            },
            "getplaylist" => {
                Self::handle_get_playlist(player, datum)
            },
            "isbusy" => {
                let is_busy = Self::handle_is_busy(player, datum)?;
                Ok(player.alloc_datum(Datum::Int(if is_busy { 1 } else { 0 })))
            },
            _ => Err(ScriptError::new(format!("No handler {handler_name} for sound channel")))
        }
    }
    ...
    pub fn get_prop(player: &DirPlayer, datum: &DatumRef, prop: &String) -> Result<Datum, ScriptError> {
        ...
        match prop.as_str() {
            ...
            _ => Err(ScriptError::new(format!("Cannot get property {} for sound channel", prop))),
        }
    }
    ...
    pub fn set_prop(player: &mut DirPlayer, datum: &DatumRef, prop: &String, value_ref: &DatumRef) -> Result<(), ScriptError> {
      match prop.as_str() {
        ...
        _ => Err(ScriptError::new(format!("Cannot set property {} for sound channel", prop))),
      }
    }
    ...
    fn get_channel_index(datum: &DatumRef, player: &DirPlayer) -> Result<usize, ScriptError> {
        ...
        match datum_val {
            Datum::SoundChannel(channel_num) => {
                ...
                if *channel_num == 0 {
                    return Err(ScriptError::new("Sound channel index must be >= 1".to_string()));
                }
                Ok((*channel_num - 1) as usize)
            }
            _ => Err(ScriptError::new("Expected sound channel reference".to_string()))
        }
    }
    
    fn get_sound_channel(player: &DirPlayer, datum: &DatumRef) -> Result<Rc<RefCell<SoundChannel>>, ScriptError> {
        let channel_idx = Self::get_channel_index(datum, player)?;
        player.sound_manager.get_channel(channel_idx)
            .ok_or_else(|| ScriptError::new(format!("Invalid sound channel {}", channel_idx + 1)))
    }

    fn get_sound_channel_mut(player: &mut DirPlayer, datum: &DatumRef) -> Result<Rc<RefCell<SoundChannel>>, ScriptError> {
        let channel_idx = Self::get_channel_index(datum, player)?;
        player.sound_manager
            .get_channel_mut(channel_idx)
            .ok_or_else(|| ScriptError::new(format!("Invalid sound channel {}", channel_idx + 1)))
    }
    
    fn handle_play_member(player: &mut DirPlayer, datum: &DatumRef, member_ref: &DatumRef) -> Result<(), ScriptError> {
    ...
    fn handle_play(player: &mut DirPlayer, datum: &DatumRef) -> Result<(), ScriptError> {
        ...
        console::log_1(&format!("🎬 handle_play() - Channel {} has {} items in playlist", 
            ch.channel_num, ch.playlist_segments.len()).into());
    ...
    fn handle_playfile(player: &mut DirPlayer, datum: &DatumRef, member_ref: &DatumRef) -> Result<DatumRef, ScriptError> {
    ...
    fn handle_queue(player: &mut DirPlayer, datum: &DatumRef, member: &DatumRef) -> Result<(), ScriptError> {
        let channel = Self::get_sound_channel_mut(player, datum)?;
    ...
    fn handle_fade_in(player: &mut DirPlayer, datum: &DatumRef, ticks: i32, to_volume: f32) -> Result<(), ScriptError> {
        let channel = Self::get_sound_channel_mut(player, datum)?;
        channel.borrow_mut().fade_in(ticks, to_volume);
        Ok(())
    }
    
    fn handle_fade_out(player: &mut DirPlayer, datum: &DatumRef, ticks: i32) -> Result<(), ScriptError> {
        let channel = Self::get_sound_channel_mut(player, datum)?;
        channel.borrow_mut().fade_out(ticks);
        Ok(())
    }
    
    fn handle_fade_to(player: &mut DirPlayer, datum: &DatumRef, ticks: i32, to_volume: f32) -> Result<(), ScriptError> {
        let channel = Self::get_sound_channel_mut(player, datum)?;
    ...
    fn handle_set_playlist(
        ...
        for (idx, segment_ref) in lingo_list.iter().enumerate() {
            ...
            if let Datum::PropList(props, _) = segment_datum {
                ...
                match (member_value, loopcount_value) {
                    ...
                    (Some(_), Some(loop_count)) if loop_count < 0 => {
                        console::log_1(&format!(
                            "  ⚠️ Skipped: invalid negative loopCount ({})",
                            loop_count
                        ).into());
                    }
                    ...
                    _ => {
                        console::log_1(&"  ⚠️ Unexpected combination of properties — skipped".into());
                    }
                }
                ...
                console::log_1(
                    &format!("  ⚠️ Skipped: item is not a PropList").into(),
                );
            }
        }
        ...
        console::log_1(&format!("✅ Built {} valid playlist entries", channel.playlist.len()).into());
        Ok(DatumRef::Void)
    }
    ...
    fn set_sound_volume(player: &mut DirPlayer, datum: &DatumRef, vol: f32) -> Result<(), ScriptError> {
        // get the Rc<RefCell<SoundChannel>>
        let channel_rc = Self::get_sound_channel(player, datum)?;
    ...
    fn set_loop_count(player: &mut DirPlayer, datum: &DatumRef, count: i32) -> Result<(), ScriptError> {
        let channel_rc = Self::get_sound_channel(player, datum)?;
        let mut channel = channel_rc.borrow_mut();
    ...
    fn set_loop_start_time(player: &mut DirPlayer, datum: &DatumRef, time: f32) -> Result<(), ScriptError> {
        let channel = Self::get_sound_channel_mut(player, datum)?;
        channel.borrow_mut().loop_start_time = time.max(0.0);
        Ok(())
    }

    fn set_loop_end_time(player: &mut DirPlayer, datum: &DatumRef, time: f32) -> Result<(), ScriptError> {
        let channel = Self::get_sound_channel_mut(player, datum)?;
            ...
            (3, 32) => {
                for frame in 0..num_frames {
                    for ch in 0..num_channels {
                        ...
                        if idx + 3 >= bytes.len() { break; }
                        let bits = u32::from_le_bytes([bytes[idx], bytes[idx + 1], bytes[idx + 2], bytes[idx + 3]]);
...
impl WebAudioBackend {
    ...
    pub fn resume_sound(&mut self) {
        console::log_1(&JsValue::from_str("▶️ resume_sound Resuming AudioContext..."));
...
impl SoundManager {
    pub fn new(num_channels: usize) -> Result<Self, ScriptError> {
        ...
        for i in 0..num_channels {
            channels.push(Rc::new(RefCell::new(SoundChannel::new(i as i32, context.clone()))));
        }
    ...
    pub fn update(&mut self, delta_time: f32, player: &mut DirPlayer) -> Result<(), ScriptError> {
        for channel in &self.channels {
            ...
            web_sys::console::log_1(&format!("[CH{}] update tick", channel.borrow().channel_num).into());
...
impl SoundChannel {
    ...
    fn find_mp3_start(data: &[u8]) -> Option<usize> {
        for i in 0..data.len().saturating_sub(4) {
            ...
            if data[i] == 0xFF && (data[i + 1] & 0xE0) == 0xE0 {
                ...
                bitrate_index != 0xF && bitrate_index != 0 &&
                sample_rate_index != 3 {
                    console::log_1(&format!(
    ...
    async fn play_current_segment_async(channel_rc: Rc<RefCell<Self>>) {
        ...
        let (member_ref, audio_context, channel_num, is_decoding) = {
            ...
            let idx = match ch.current_segment_index { Some(i) => i, None => { console::log_1(&"⚠️ No current segment to play".into()); return; } };
            let seg = match ch.playlist_segments.get(idx) { Some(s) => s.clone(), None => { console::log_1(&"⚠️ Invalid segment index".into()); return; } };
            (seg.member_ref.clone(), ch.audio_context.clone(), ch.channel_num, ch.is_decoding.clone())
        };

        // If decode in-progress, skip
        if *is_decoding.borrow() {
            console::log_1(&format!("⏳ Channel {} is currently decoding — skipping play_current_segment_async", channel_num).into());
            return;
        }
    ...
    async fn play_async(
        ...
        let (audio_buffer, _channels, _sample_rate) = channel_rc.borrow_mut().load_audio_data(&*audio_context, &member_ref).await?;
        ...
        source_node.connect_with_audio_node(&gain_node)
            .map_err(|e| JsValue::from_str(&format!("Failed to connect source to gain: {:?}", e)))?;

        gain_node.connect_with_audio_node(&panner_node)
            .map_err(|e| JsValue::from_str(&format!("Failed to connect gain to panner: {:?}", e)))?;

        // FIX: Dereferencing Arc<AudioContext> to get destination
        panner_node.connect_with_audio_node(&audio_context.destination())
            .map_err(|e| JsValue::from_str(&format!("Failed to connect panner to destination: {:?}", e)))?;
    ...
    fn resolve_sound_member(player: &DirPlayer, datum: &Datum) -> Option<SoundMember> {
        ...
        match member_datum {
            ...
            other => {
                console::log_1(&format!("⚠️ #member is not a CastMember, it's {:?}", other.type_str()).into());
                None
            }
        }
    }
    ...
    pub fn play_file(self_rc: Rc<RefCell<Self>>, member_ref: DatumRef) {
        console::log_1(&format!("▶️ SoundChannel::play_file() called with {:?}", member_ref).into());

        {
            let mut channel = self_rc.borrow_mut();
            web_sys::console::log_1(&format!("play_file(): channel={} status={:?} queued={:?} current_idx={:?}", 
                channel.channel_num, channel.status, channel.queued_member, channel.current_segment_index).into());
            ...
            if channel.current_segment_index.is_none() {
                for (idx, segment) in channel.playlist_segments.iter().enumerate() {
                    if segment.member_ref == member_ref {
                        ...
                        web_sys::console::log_1(&format!("📍 Set current_segment_index to {}", idx).into());
                        break;
                    }
                }
            } else {
                web_sys::console::log_1(&format!("🔒 Keeping existing current_segment_index={:?}", channel.current_segment_index).into());
            }
        }
    ...
    pub fn start_sound(self_rc: Rc<RefCell<Self>>, member_ref: DatumRef) {
        ...
        console::log_1(&format!("▶️ SoundChannel::start_sound() called with {:?}", member_ref).into());
        ...
        if let Some(sound_member) = Self::resolve_sound_member(player, &datum) {
            ...
            if let Some(mp3_bytes) = &audio_data.compressed_data {
                ...
                wasm_bindgen_futures::spawn_local(async move {
                    ...
                    if let Err(e) = Self::start_sound_mp3_async(self_rc_clone.clone(), mp3_data).await {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                let resampled_buffer = match Self::resample_audio_buffer(&source_buffer, target_sample_rate).await {
                    Ok(buf) => {
                        console::log_1(&format!("✅ Resampled to {} Hz", buf.sample_rate()).into());
                        buf
                    }
                    Err(e) => {
                        console::log_1(&format!("⚠️ Resampling failed: {:?}, using original", e).into());
                        source_buffer
                    }
                };
                ...
                let closure = Closure::<dyn FnMut()>::new(move || {
                    web_sys::console::log_1(&format!("🔚 Channel {} sound ended", channel_num).into());
    ...
    async fn resample_audio_buffer(buffer: &AudioBuffer, target_rate: f32) -> Result<AudioBuffer, JsValue> {
        ...
        // Render the audio
        let render_promise = offline.start_rendering()
            .map_err(|e| {
    ...
    async fn start_sound_mp3_async(
        ...
        // Guard: prevent re-entrant decode/play
        {
            ...
            if *ch.is_decoding.borrow() {
                console::log_1(&format!(
                    "⚠️ Channel {} already decoding – skipping", 
                    ch.channel_num
                ).into());
                return Ok(());
            }
        ...
        if clean_mp3.len() >= 4 {
            let header = u32::from_be_bytes([clean_mp3[0], clean_mp3[1], clean_mp3[2], clean_mp3[3]]);
            let version = (header >> 19) & 0x3;
            let layer = (header >> 17) & 0x3;
            let bitrate_index = (header >> 12) & 0xF;
            let sample_rate_index = (header >> 10) & 0x3;
            ...
            if clean_mp3.len() < 100 {
                console::log_1(&format!("⚠️ MP3 data suspiciously small: {} bytes", clean_mp3.len()).into());
            }
        }
        ...
        let decoded_js = match wasm_bindgen_futures::JsFuture::from(decode_promise).await {
            ...
            Err(e) => {
                ...
                console::log_1(&format!("📊 Data size: {} bytes, First 16 bytes: {:02X?}", 
                    clean_mp3.len(), &clean_mp3[0..16.min(clean_mp3.len())]).into());
                console::log_1(&"ℹ️ This might be Director-specific encoding. Falling back to PCM.".into());
                clear_flag();
                return Err(e);
            }
        };
        ...
        let final_buffer = if let Some(expected_rate) = expected_rate_opt {
            ...
            if (audio_buffer.sample_rate() - expected_rate_f).abs() > 1.0 {
                console::log_1(&format!(
                    "🔄 Resampling {} -> {} Hz",
                    audio_buffer.sample_rate(), expected_rate_f
                ).into());
                
                match Self::resample_audio_buffer(&audio_buffer, expected_rate_f).await {
                    Ok(rbuf) => {
                        console::log_1(&format!("✅ Resampled buffer: {} samples", rbuf.length()).into());
                        rbuf
                    }
                    Err(e) => {
                        console::log_1(&format!("⚠️ Resample failed: {:?}, using original", e).into());
                        audio_buffer
                    }
                }
            } else {
        ...
        let volume = { let ch = self_rc.borrow(); ch.volume };
        gain.gain().set_value(volume / 255.0);
        console::log_1(&format!("🔊 Setting gain to {} (volume: {})", volume / 255.0, volume).into());

        // Create pan node if available
        if let Ok(pan_node) = ctx.create_stereo_panner() {
            let pan_value = { let ch = self_rc.borrow(); ch.pan };
        ...
        if let Some(sound_member) = Self::resolve_sound_member(player, &datum) {
            ...
            for ch in 0..audio_data.num_channels {
                ...
                for frame in 0..resampled_frames {
                    ...
                    let idx2 = ((source_frame + 1) * audio_data.num_channels as usize + ch as usize)
                        .min(audio_data.samples.len() - 1);
            ...
            let self_rc_clone = self_rc.clone();
            let closure = Closure::<dyn FnMut()>::new(move || {
                web_sys::console::log_1(&format!("🔚 Channel {} PCM sound ended", channel_num).into());
    ...
    fn get_mp3_frame_info(header: &[u8; 4]) -> Option<(usize, u32)> {
        ...
        let frame_size = ((samples_per_frame / 8 * bitrate * 1000 / sample_rate) + padding as u32) as usize;
...
// }
    ...
    pub fn load_director_sound_from_bytes(
        ...
        let (is_mp3, is_probably_adpcm, is_probably_8bit) = if codec == "raw_pcm" {
            console::log_1(&"🔒 Codec is 'raw_pcm' - forcing PCM processing, no format detection".into());
            ...
            let is_probably_8bit = !is_mp3 && (bits_per_sample == 8 || (
                data.len() >= 100 && 
                data[0..100].iter().filter(|&&b| b >= 0x60 && b <= 0xA0).count() > 50
            ));
        ...
        let pcm_data = if is_mp3 {
            ...
            let decoded_pcm_samples = Self::decode_ima_adpcm_to_pcm(
                adpcm_samples,
                initial_predictor,
                initial_index,
            )?;
            ...
            console::log_1(&format!(
                "✅ ADPCM decoded: {} samples",
                decoded_pcm_samples.len()
            ).into());
            ...
            if bits_per_sample == 16 {
                ...
                let looks_like_8bit_unsigned = data.len() >= 100 && 
                    data[0..100].iter().filter(|&&b| b >= 0x40 && b <= 0xBF).count() > 80;
                
                if looks_like_8bit_unsigned {
                    console::log_1(&"🔍 Detected 8-bit unsigned samples - converting to 16-bit signed".into());
        ...
        // --- STEP 4: Build valid RIFF/WAV ---
        let bits = if (is_probably_8bit || is_probably_adpcm) && bits_per_sample != 16 { 16 } else { bits_per_sample };
        let sample_count = (pcm_data.len() / (channels as usize * (bits as usize / 8))) as u32;
        let byte_rate = sample_rate * channels as u32 * bits as u32 / 8;
        let block_align = (channels * bits / 8) as u16;
        let data_len = pcm_data.len() as u32;
        ...
        console::log_1(&format!(
            ...
            wav.len(), sample_rate, bits, channels
        ).into());
    ...
    pub fn decode_ima_adpcm_to_pcm(
        ...
        for &byte in adpcm_data.iter() {
            ...
            if (lower_nibble & 0x1) != 0 { diff += step; }
            if (lower_nibble & 0x2) != 0 { diff += step >> 1; }
            if (lower_nibble & 0x4) != 0 { diff += step >> 2; }
            ...
            let mut diff: i32 = step >> 3; // Start with step/8
            if (upper_nibble & 0x1) != 0 { diff += step; }
            if (upper_nibble & 0x2) != 0 { diff += step >> 1; }
            if (upper_nibble & 0x4) != 0 { diff += step >> 2; }
    ...
    pub fn load_director_audio_data(
        ...
        if codec == "raw_pcm" {
            console::log_1(&"📝 Codec is 'raw_pcm' - treating as PCM, skipping MP3 detection".into());
    ...
    pub async fn play_mp3_data(mp3_bytes: &[u8]) -> Result<(), JsValue> {
        console::log_1(&format!("🎵 Creating Blob from {} bytes of MP3 data", mp3_bytes.len()).into());
    ...
    pub async fn decode_mp3_to_wav(
        ...
        let buffer = audio_buffer.dyn_into::<AudioBuffer>()
            .map_err(|e| {
    ...
    pub async fn play_mp3_via_web_audio(
        &mut self,
        mp3_bytes: &[u8],
    ) -> Result<(), JsValue> {
    ...
    pub fn load_director_sound(data: &[u8]) -> Result<AudioData, String> {
        ...
        if Self::is_wav_format(data) && data.len() >= 44 {
            ...
            match Self::load_wav(data) {
                Ok(audio) if audio.sample_rate > 0 && !audio.samples.is_empty() => return Ok(audio),
                _ => {
    ...
    pub fn queue(&mut self, datum_ref: DatumRef, player: &DirPlayer) {
        ...
        let props = match datum {
            ...
            _ => {
                console::log_1(&"⚠️ queue(): called with non-propList or empty list — ignored".into());
                return;
            }
        };
        ...
        if loop_count <= 0 {
            console::log_1(
                &format!("⚠️ queue(): invalid loopCount={} — skipping entry", loop_count).into(),
            );
            return;
        }
    ...
    pub fn set_playlist(&mut self, list: Vec<DatumRef>, player: &DirPlayer) {
        ...
        if list.is_empty() {
            web_sys::console::log_1(&format!("🧹 Cleared playlist for channel {}", self.channel_num).into());
            return;
        }
    ...
    pub fn start_segment_playback(&mut self, sound_member: &SoundMember, _loop_count: i32) -> Result<(), ScriptError> {
        self.sound_member = Some(sound_member.clone());
        ...
        for channel in 0..audio_data.num_channels {
            let mut channel_data = buffer.get_channel_data(channel as u32)
        ...
        if let Some(ref pan_node) = pan {
            source.connect_with_audio_node(pan_node)
                .map_err(|e| ScriptError::new(format!("Failed to connect source to pan: {:?}", e)))?;
            pan_node.connect_with_audio_node(&gain)
            ...
            source.connect_with_audio_node(&gain)
                .map_err(|e| ScriptError::new(format!("Failed to connect source to gain: {:?}", e)))?;
        }
        
        gain.connect_with_audio_node(&self.audio_context.destination())
            .map_err(|e| ScriptError::new(format!("Failed to connect gain to destination: {:?}", e)))?;
    ...
    pub fn play_member(&mut self, player: &mut DirPlayer, member_ref: &DatumRef, loop_count: i32) -> Result<(), ScriptError> {
        // Clear playlist
        self.playlist.clear();
        self.current_segment_index = None;
    ...
    pub async fn play_castmember(
        ...
        console::log_1(&format!("🎵 play_castmember: {} frames @ {}Hz", 
            samples.len() / num_channels as usize, sample_rate).into());
    ...
    pub async fn play_sound(&mut self, audio_data: &AudioData) -> Result<(), JsValue> {
        use web_sys::{Blob, Url, HtmlAudioElement};
   ...
   pub fn stop_sound(&mut self) {
    ...
    pub fn start_next_segment(&mut self) {
        ...
        console::log_1(&format!("🔄 start_next_segment called for channel {}", self.channel_num).into());
        ...
        let index = match self.current_segment_index {
            ...
            None => {
                ...
                if !self.playlist_segments.is_empty() {
                    console::log_1(&format!("🎵 Found {} queued sounds", self.playlist_segments.len()).into());
                    self.current_segment_index = Some(0);
   ...
   fn trigger_playback_for_current_segment(&mut self) {
        ...
        if let Err(e) = self.play(member_ref, loop_count_val) {
             console::error_1(&JsValue::from_str(&format!("Failed to trigger playback: {:?}", e)));
        }
    }
   ...
   async fn load_audio_data(
        ...
        console::log_1(&format!("Audio buffer created at {} Hz. Context is {} Hz.", 
            buffer_sample_rate, 
