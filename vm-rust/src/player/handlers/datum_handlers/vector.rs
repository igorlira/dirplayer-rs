...
impl VectorDatumHandlers {
    ...
    pub fn call(
        ...
        match handler_name.into_builtin() {
            ...
            Some(BuiltInSymbol::Duplicate) => reserve_player_mut(|player| {
                let vec = Self::datum_to_vec(player, player.get_datum(datum))?;
                Ok(player.alloc_datum(Datum::Vector(vec)))
            }),
            Some(BuiltInSymbol::DistanceTo) => reserve_player_mut(|player| {
            ...
            Some(BuiltInSymbol::GetNormalized) => reserve_player_mut(|player| {
            ...
            Some(BuiltInSymbol::Normalize) => reserve_player_mut(|player| {
                let [x, y, z] = Self::datum_to_vec(player, player.get_datum(datum))?;
                let len = (x*x + y*y + z*z).sqrt();
                if len > 1e-10 {
            ...
            Some(BuiltInSymbol::DotProduct | BuiltInSymbol::Dot) => reserve_player_mut(|player| {
                let a = Self::datum_to_vec(player, player.get_datum(datum))?;
                let b = Self::datum_to_vec(player, player.get_datum(&args[0]))?;
                Ok(player.alloc_datum(Datum::Float(a[0]*b[0] + a[1]*b[1] + a[2]*b[2])))
            }),
            Some(BuiltInSymbol::AngleBetween) => reserve_player_mut(|player| {
                let a = Self::datum_to_vec(player, player.get_datum(datum))?;
                let b = Self::datum_to_vec(player, player.get_datum(&args[0]))?;
