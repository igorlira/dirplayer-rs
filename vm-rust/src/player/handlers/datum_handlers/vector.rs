use crate::{
    ...
    player::{reserve_player_mut, DirPlayer, ScriptError, DatumRef},
};
...
impl VectorDatumHandlers {
    ...
    pub fn get_prop(player: &DirPlayer, datum: &DatumRef, prop: &str) -> Result<Datum, ScriptError> {
        ...
        match prop {
            ...
            _ => Err(ScriptError::new(format!("Cannot get vector property {}", prop))),
        }
    }
    ...
    pub fn set_prop(player: &mut DirPlayer, datum: &DatumRef, prop: &str, value_ref: &DatumRef) -> Result<(), ScriptError> {
        ...
        match prop {
            ...
            "y" => vec[1] = value,
            "z" => vec[2] = value,
            _ => return Err(ScriptError::new(format!("Cannot set vector property {}", prop))),
        }
    ...
    pub fn add(a: &DatumRef, b: &DatumRef) -> Result<DatumRef, ScriptError> {
        reserve_player_mut(|player| {
            ...
            Ok(Self::vec_to_datum(player, [va[0]+vb[0], va[1]+vb[1], va[2]+vb[2]]))
        })
    }
    ...
    pub fn mul(a: &DatumRef, b: &DatumRef) -> Result<DatumRef, ScriptError> {
        reserve_player_mut(|player| {
            ...
            match player.get_datum(b) {
                Datum::Float(f) => Ok(Self::vec_to_datum(player, [va[0]*(*f as f32), va[1]*(*f as f32), va[2]*(*f as f32)])),
                Datum::Vector(vb) => Ok(Self::vec_to_datum(player, [va[0]*vb[0], va[1]*vb[1], va[2]*vb[2]])),
                _ => Err(ScriptError::new("Invalid operand for vector multiplication".to_string())),
            }
        })
    }
    ...
    pub fn div(a: &DatumRef, b: &DatumRef) -> Result<DatumRef, ScriptError> {
        reserve_player_mut(|player| {
            ...
            match player.get_datum(b) {
                Datum::Float(f) => {
                    if *f == 0.0 { return Err(ScriptError::new("Division by zero".to_string())); }
                    Ok(Self::vec_to_datum(player, [va[0]/(*f as f32), va[1]/(*f as f32), va[2]/(*f as f32)]))
                }
                Datum::Vector(vb) => {
                    if vb[0] == 0.0 || vb[1] == 0.0 || vb[2] == 0.0 {
                        return Err(ScriptError::new("Division by zero in vector components".to_string()));
                    }
                    Ok(Self::vec_to_datum(player, [va[0]/vb[0], va[1]/vb[1], va[2]/vb[2]]))
                }
                _ => Err(ScriptError::new("Invalid operand for vector division".to_string())),
            }
        })
    }
}
