use crate::{director::{chunks::handler::Bytecode, lingo::datum::{datum_bool, Datum, StringChunkExpr, StringChunkSource, StringChunkType}}, player::{context_vars::{player_get_context_var, player_set_context_var, read_context_var_args}, datum_formatting::format_concrete_datum, handlers::datum_handlers::string_chunk::StringChunkUtils, reserve_player_mut, DirPlayer, HandlerExecutionResult, HandlerExecutionResultContext, ScriptError}};
...
impl StringBytecodeHandler {
  ...
  pub fn contains_str(_: &Bytecode, ctx: &BytecodeHandlerContext) -> Result<HandlerExecutionResultContext, ScriptError> {
  ...
  pub fn join_pad_str(_: &Bytecode, ctx: &BytecodeHandlerContext) -> Result<HandlerExecutionResultContext, ScriptError> {
    reserve_player_mut(|player| {
  ...
  pub fn join_str(_: &Bytecode, ctx: &BytecodeHandlerContext) -> Result<HandlerExecutionResultContext, ScriptError> {
    reserve_player_mut(|player| {
      let (left_id, right_id) = {
        let scope = player.scopes.get_mut(ctx.scope_ref).unwrap();
        let right = scope.stack.pop().unwrap();
  ...
  pub fn put(bytecode: &Bytecode, ctx: &BytecodeHandlerContext) -> Result<HandlerExecutionResultContext, ScriptError> {
    reserve_player_mut(|player| {
      ...
      match put_type {
        ...
        PutType::Before => {
          let curr_string_id = player_get_context_var(player, id_ref, cast_id_ref, var_type, &ctx)?;
          ...
          player_set_context_var(player, id_ref, cast_id_ref, var_type, new_string, &ctx)?;
        }
        PutType::After => {
          let curr_string_id = player_get_context_var(player, id_ref, cast_id_ref, var_type, &ctx)?;
          let curr_string = player.get_datum(curr_string_id);
  ...
  fn read_chunk_ref(player: &mut DirPlayer, ctx: &BytecodeHandlerContext) -> Result<StringChunkExpr, ScriptError> {
  ...
  pub fn get_chunk(_: &Bytecode, ctx: &BytecodeHandlerContext) -> Result<HandlerExecutionResultContext, ScriptError> {
    reserve_player_mut(|player| {
      let string = {
        let scope = player.scopes.get_mut(ctx.scope_ref).unwrap();
        scope.stack.pop().unwrap()
  ...
  pub fn delete_chunk(bytecode: &Bytecode, ctx: &BytecodeHandlerContext) -> Result<HandlerExecutionResultContext, ScriptError> {
    reserve_player_mut(|player| {
      let (id_ref, cast_id_ref) = read_context_var_args(player, bytecode.obj as u32, ctx.scope_ref);
      let string_ref = player_get_context_var(player, id_ref, cast_id_ref, bytecode.obj as u32, ctx)?;
  ...
  pub fn contains_0str(_: &Bytecode, ctx: &BytecodeHandlerContext) -> Result<HandlerExecutionResultContext, ScriptError> {
    reserve_player_mut(|player| {
