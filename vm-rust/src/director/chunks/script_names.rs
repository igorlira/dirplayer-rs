...
impl ScriptNamesChunk {
  ...
  pub fn from_reader(reader: &mut BinaryReader, dir_version: u16) -> Result<ScriptNamesChunk, String> {
    ...
    let names = (0..names_count).map(|_| reader.read_pascal_string().unwrap()).collect();
