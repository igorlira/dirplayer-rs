...
impl ScriptContextMapEntry {
  ...
  pub fn from_reader(reader: &mut BinaryReader, dir_version: u16) -> Result<ScriptContextMapEntry, String> {
...
impl ScriptContextChunk {
  #[allow(unused_variables)]
  pub fn from_reader(reader: &mut BinaryReader, dir_version: u16) -> Result<ScriptContextChunk, String> {
    ...
    let section_map = (0..entry_count).map(|_| ScriptContextMapEntry::from_reader(reader, dir_version).unwrap()).collect();
