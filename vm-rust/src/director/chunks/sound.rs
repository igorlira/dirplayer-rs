...
impl SoundChunk {
    ...
    pub fn set_metadata(&mut self, sample_rate: u32, channels: u16, bits_per_sample: u16) {
        ...
        self.sample_count = (self.data.len() / (channels as usize * (bits_per_sample / 8) as usize)) as u32;
...
impl SoundChunk {
    pub fn from_snd_chunk(reader: &mut BinaryReader) -> Result<SoundChunk, String> {
        ...
        let hex_dump = data_test
            ...
            .collect::<Vec<String>>()
            .join(" ");
        console::log_1(&format!("WAV Hex Dump (Full File, {} bytes):\n{}", data_test.len(), hex_dump).into());
        ...
        let _ = reader.read_u32().map_err(|e| format!("Failed to read 0x00: {}", e))?; // Skip 0x00-0x03

        // Offset 0x04 (4 bytes): Sample Rate ENCODED (Rate A - 16000 Hz target)
        let sample_rate_encoded = reader.read_u32().map_err(|e| format!("Failed to read 0x04: {}", e))?;
        
        // Calculate 16000 Hz target rate and snap to 16000
        let mut rate_a = (sample_rate_encoded as f64 / 6.144).round() as u32;
        if rate_a > 15990 && rate_a < 16020 { rate_a = 16000; } 
        
        // Skip to 0x16 (We are at 0x08, need to skip 14 bytes)
        for _ in 0..14 {
            let _ = reader.read_u8().map_err(|e| format!("Failed to skip to 0x16: {}", e))?;
        }

        // Offset 0x16 (2 bytes): Sample Rate u16 (Rate B - 22050 Hz target)
        let rate_b_u16 = reader.read_u16().map_err(|e| format!("Failed to read 0x16: {}", e))?;
        let rate_b = rate_b_u16 as u32;

        // Skip to 0x2A (We are at 0x18, need to skip 18 bytes)
        for _ in 0..(0x2A - 0x18) {
            let _ = reader.read_u8().map_err(|e| format!("Failed to skip to 0x2A: {}", e))?;
        }

        // Offset 0x2A (2 bytes): Sample Rate u16 (Rate C - 44100 Hz target)
        let rate_c_u16 = reader.read_u16().map_err(|e| format!("Failed to read 0x2A: {}", e))?;
        ...
        for _ in 0..(0x40 - 0x2C) {
            let _ = reader.read_u8().map_err(|e| format!("Failed to skip remaining header: {}", e))?;
        }
        ...
        console::log_1(&format!(
            ...
            sample_rate, bits_per_sample, data.len(), sample_count, duration
        ).into());
    ...
    pub fn from_media(media: &MediaChunk) -> SoundChunk {
        ...
        console::log_1(&format!(
            ...
            codec, media.data_size_field, media.audio_data.len(), sample_count
        ).into());
