...
impl ConfigChunk {
  pub fn from_reader(reader: &mut BinaryReader, _dir_version: u16, dir_endian: Endian) -> Result<ConfigChunk, String> {
    ...
    /* directorVersion = */ reader.read_u16().unwrap();
    ...
    if checksum != computed_checksum {
      log_i(format!("Checksums don't match! Stored: {checksum} Computed: {computed_checksum}").as_str());
    }
