...
impl ScoreFrameChannelData {
  pub fn read(reader: &mut BinaryReader) -> Result<ScoreFrameChannelData, String> {
    let sprite_type = reader.read_u8().map_err(|e| format!("Failed to read sprite_type: {:?}", e))?;
    let ink = reader.read_u8().map_err(|e| format!("Failed to read ink: {:?}", e))?;
    let fore_color = reader.read_u8().map_err(|e| format!("Failed to read fore_color: {:?}", e))?;
    let back_color = reader.read_u8().map_err(|e| format!("Failed to read back_color: {:?}", e))?;
    let cast_lib = reader.read_u16().map_err(|e| format!("Failed to read cast_lib: {:?}", e))?;
    let cast_member = reader.read_u16().map_err(|e| format!("Failed to read cast_member: {:?}", e))?;
    let unk1 = reader.read_u16().map_err(|e| format!("Failed to read unk1: {:?}", e))?;
    let unk2 = reader.read_u16().map_err(|e| format!("Failed to read unk2: {:?}", e))?;
    let pos_y = reader.read_u16().map_err(|e| format!("Failed to read pos_y: {:?}", e))?;
    let pos_x = reader.read_u16().map_err(|e| format!("Failed to read pos_x: {:?}", e))?;
    let height = reader.read_u16().map_err(|e| format!("Failed to read height: {:?}", e))?;
    let width = reader.read_u16().map_err(|e| format!("Failed to read width: {:?}", e))?;
...
impl ScoreFrameData {
  ...
  pub fn read(reader: &mut BinaryReader) -> Result<ScoreFrameData, String> {
    ...
    while !reader.eof() {
      let length = reader.read_u16().map_err(|e| format!("Failed to read frame length: {:?}", e))?;
      ...
      if frame_length > 0 {
        let chunk_data = reader.read_bytes(frame_length as usize).map_err(|e| format!("Failed to read chunk data: {:?}", e))?;
        ...
        while !frame_chunk_reader.eof() {
          ...
          let channel_size = frame_chunk_reader.read_u16().map_err(|e| format!("Failed to read channel size: {:?}", e))? as usize;
          let channel_offset = frame_chunk_reader.read_u16().map_err(|e| format!("Failed to read channel offset: {:?}", e))? as usize;
          let channel_delta = frame_chunk_reader.read_bytes(channel_size).map_err(|e| format!("Failed to read channel delta: {:?}", e))?;
          ...
          channel_data[frame_offset + channel_offset..end_offset].copy_from_slice(&channel_delta);
        }
      }
  ...
  fn read_header(reader: &mut BinaryReader) -> Result<ScoreFrameDataHeader, String> {
    let actual_length = reader.read_u32().map_err(|e| format!("Failed to read actual_length: {:?}", e))?;
    let unk1 = reader.read_u32().map_err(|e| format!("Failed to read unk1: {:?}", e))?;
    let frame_count = reader.read_u32().map_err(|e| format!("Failed to read frame_count: {:?}", e))?;
    let frames_version = reader.read_u16().map_err(|e| format!("Failed to read frames_version: {:?}", e))?;
    let sprite_record_size = reader.read_u16().map_err(|e| format!("Failed to read sprite_record_size: {:?}", e))?;
    let num_channels = reader.read_u16().map_err(|e| format!("Failed to read num_channels: {:?}", e))?;
    let _num_channels_displayed: u16;

    if frames_version > 13 {
      _num_channels_displayed = reader.read_u16().map_err(|e| format!("Failed to read _num_channels_displayed: {:?}", e))?;
    } else {
      if frames_version <= 7 {
      ...
      reader.read_u16().map_err(|e| format!("Failed to skip u16: {:?}", e))?;  // Skip
    }
...
impl FrameIntervalPrimary {
  pub fn read(reader: &mut BinaryReader) -> Result<FrameIntervalPrimary, String> {
    let start_frame = reader.read_u32().map_err(|e| format!("Failed to read start_frame: {:?}", e))?;
    let end_frame = reader.read_u32().map_err(|e| format!("Failed to read end_frame: {:?}", e))?;
    let unk0 = reader.read_u32().map_err(|e| format!("Failed to read unk0: {:?}", e))?;
    let unk1 = reader.read_u32().map_err(|e| format!("Failed to read unk1: {:?}", e))?;
    let channel_index = reader.read_u32().map_err(|e| format!("Failed to read channel_index: {:?}", e))?;
    let unk2 = reader.read_u16().map_err(|e| format!("Failed to read unk2: {:?}", e))?;
    let unk3 = reader.read_u32().map_err(|e| format!("Failed to read unk3: {:?}", e))?;
    let unk4 = reader.read_u16().map_err(|e| format!("Failed to read unk4: {:?}", e))?;
    let unk5 = reader.read_u32().map_err(|e| format!("Failed to read unk5: {:?}", e))?;
    let unk6 = reader.read_u32().map_err(|e| format!("Failed to read unk6: {:?}", e))?;
    let unk7 = reader.read_u32().map_err(|e| format!("Failed to read unk7: {:?}", e))?;
    let unk8 = reader.read_u32().map_err(|e| format!("Failed to read unk8: {:?}", e))?;
...
impl ScoreChunkHeader {
  pub fn read(reader: &mut BinaryReader) -> Result<Self, String> {
    Ok(ScoreChunkHeader {
      total_length: reader.read_u32().map_err(|e| format!("Failed to read total_length: {:?}", e))?,
      unk1: reader.read_u32().map_err(|e| format!("Failed to read unk1: {:?}", e))?,
      unk2: reader.read_u32().map_err(|e| format!("Failed to read unk2: {:?}", e))?,
      entry_count: reader.read_u32().map_err(|e| format!("Failed to read entry_count: {:?}", e))?,
      unk3: reader.read_u32().map_err(|e| format!("Failed to read unk3: {:?}", e))?, // entry_count + 1
      entry_size_sum: reader.read_u32().map_err(|e| format!("Failed to read entry_size_sum: {:?}", e))?,
    })
  }
}
...
impl ScoreChunk {
  ...
  pub fn read(reader: &mut BinaryReader, dir_version: u16) -> Result<Self, String> {
    ...
    let header = ScoreChunkHeader::read(reader).map_err(|e| format!("Failed to read ScoreChunkHeader: {}", e))?;

    // Read offsets table
    let offsets_result: Result<Vec<usize>, String> = (0..header.entry_count + 1)
      .map(|i| {
        let offset = reader.read_u32().map_err(|e| format!("Failed to read offset: {:?}", e))? as usize;
        Ok(offset)
      })
      .collect();
    ...
    if offsets.len() != (header.entry_count + 1) as usize {
      console::error_1(&format!("❌ Offsets count mismatch! Expected {}, Got {}", header.entry_count + 1, offsets.len()).into());
      return Err("Offsets count mismatch.".to_string());
    }
  
    for j in 0..offsets.len().saturating_sub(1) {
      if offsets[j] > offsets[j + 1] {
        console::error_1(&format!("❌ Offsets are not monotonic: offsets[{}] ({}) > offsets[{}] ({})", j, offsets[j], j + 1, offsets[j + 1]).into());
  ...
  fn analyze_behavior_attachment_entries(entries: &Vec<Vec<u8>>) -> Result<Vec<(FrameIntervalPrimary, Option<FrameIntervalSecondary>)>, String> {
    let mut results = vec![];
      ...
      match entry_bytes.len() {
        44 => {
          ...
          if let Ok(primary) = FrameIntervalPrimary::read(&mut reader) {
            log_i(&format!("🎯 Found primary at entry {}: channel={}, frames={}-{}", 
            ...
            while j < entries.len() {
              ...
              if next_size >= 8 && next_size % 8 == 0 {
                ...
                log_i(&format!("  📦 Entry {} has {} bytes = {} potential behaviors", 
                    ...
                    if let Ok(cast_member) = sec_reader.read_u16() {
                      if let Ok(unk0) = sec_reader.read_u32() {
                        ...
                        if cast_lib > 0 && cast_member > 0 {
                          ...
                          // Handle parameters
                          if secondary.unk0 > 0 && (secondary.unk0 as usize) < entries.len() {
                            let proplist_idx = secondary.unk0 as usize;
                            if let Ok(proplist_string) = String::from_utf8(entries[proplist_idx].clone()) {
                              let clean = proplist_string.trim_end_matches('\0');
                              if clean.starts_with('[') {
                                let player = unsafe { PLAYER_OPT.as_mut().unwrap() };
                                // TODO: Replace `eval_lingo` with a parser
                                if let Ok(proplist) = eval_lingo(clean.to_owned(), player) {
                          ...
                          log_i(&format!("    ✅ Behavior {}: cast={}/{}, unk0={}", 
                            behavior_idx + 1, cast_lib, cast_member, unk0));
            ...
            log_i(&format!("📊 Primary for channel {} has {} behaviors total", primary.channel_index, secondaries.len()));
    ...
    log_i(&format!("🏁 Finished analyzing. Created {} results", results.len()));
    Ok(results)
  }
}
// Frame Labels
...
impl FrameLabelsChunk {
  pub fn from_reader(reader: &mut BinaryReader, _dir_version: u16) -> Result<FrameLabelsChunk, String> {
    ...
    let labels_count = reader.read_u16().map_err(|e| format!("Error reading labels_count: {:?}", e))? as usize;
    let label_frames: Vec<(usize, usize)> = (0..labels_count)
      .map(|_i| {
        let frame_num = reader.read_u16().map_err(|e| format!("Error reading frame_num: {:?}", e))? as usize;
        let label_offset = reader.read_u16().map_err(|e| format!("Error reading label_offset: {:?}", e))? as usize;
        Ok((label_offset, frame_num))
      })
      .collect::<Result<Vec<_>, String>>()?;

    let labels_size: usize = reader.read_u32().map_err(|e| format!("Error reading labels_size: {:?}", e))? as usize;
    let labels: Vec<FrameLabel> = (0..labels_count)
      .map(|i| {
          ...
          let label_str = reader.read_string(label_len).map_err(|e| format!("Error reading label_str: {:?}", e))?;
