...
impl ScriptChunk {
  ...
  pub fn from_reader(reader: &mut BinaryReader, dir_version: u16, capital_x: bool) -> Result<ScriptChunk, String> {
    ...
    let handler_records = (0..handlers_count)
      .map(|_| {
        HandlerRecord::read_record(reader, dir_version, capital_x).unwrap()
      })
      .collect_vec();
    ...
    let handlers = handler_records
      .iter()
      .map(|record| {
        HandlerRecord::read_data(reader, record).unwrap()
      })
      .collect_vec();
