...
impl XMediaChunk {
    ...
    fn looks_like_bitmap_data(data: &[u8], bytes_per_glyph: usize) -> bool {
        ...
        console::log_1(&format!(
            ...
            very_low_ratio * 100.0, low_ratio * 100.0, high as f32 / sample_size as f32 * 100.0
        ).into());
        true
    }
    ...
    pub fn parse_pfr_font(&self) -> Option<PfrFont> {
        ...
        // Extract font name
        let font_name = self.extract_font_name().unwrap_or_else(|| {
            format!("Unknown_PFR_Font")
        });
        ...
        for &offset in &candidate_offsets {
            if offset + expected_bitmap_bytes <= self.raw_data.len() {
                ...
                console::log_1(&format!(
                    ...
                    offset, expected_bitmap_bytes, self.raw_data.len().saturating_sub(offset)
                ).into());
            }
        }
        ...
        if glyph_data.is_none() {
            ...
            while scan_offset + expected_bitmap_bytes <= self.raw_data.len() {
                ...
                let low_bytes = candidate[0..sample_size].iter().filter(|&&b| b < 0x80).count();
        ...
        if glyph_data.is_none() {
            ...
            if glyph_data_offset < self.raw_data.len() {
                ...
                console::log_1(&format!(
                    ...
                    total_glyphs, char_width, char_height, vector_data.len()
                ).into());
                ...
                if rendered_bitmap.len() >= expected_bitmap_bytes {
                    ...
                    console::log_1(&format!(
                        ...
                        rendered_bitmap.len(), expected_bitmap_bytes
                    ).into());
                }
            }
        }
    ...
    pub fn decode_pfr_rle_bitmap(
        ...
        console::log_1(&format!(
            ...
            glyph_data.len(), expected_width, expected_height
        ).into());
    ...
    pub fn parse_pfr_font_with_rle(&self) -> Option<PfrFont> {
        ...
        let decoded_bitmap = Self::decode_pfr_rle_bitmap(
            rle_data,
            bitmap_width,
            bitmap_height,
        );
        ...
        console::log_1(&format!(
            ...
            char_width, char_height, grid_columns, grid_rows, decoded_bitmap.len()
        ).into());
