...
impl HandlerRecord {
  ...
  pub fn read_data(
    ...
    let argument_name_ids = read_varnames_table(reader, record.argument_count as usize, record.argument_offset);
    let local_name_ids = read_varnames_table(reader, record.locals_count as usize, record.locals_offset);
    let global_name_ids = read_varnames_table(reader, record.globals_count as usize, record.globals_offset);
