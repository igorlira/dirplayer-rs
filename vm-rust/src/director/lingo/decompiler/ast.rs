...
impl Datum {
    ...
    pub fn write_script(&self, code: &mut CodeWriter, dot: bool, _sum: bool) {
        match self.datum_type {
            ...
            DatumType::List => {
                ...
                for (i, item) in self.list_value.iter().enumerate() {
                    ...
                    item.write_script(code, dot, false);
                }
                code.write("]");
            }
            ...
            DatumType::PropList => {
                ...
                while i + 1 < self.list_value.len() {
                    ...
                    self.list_value[i].write_script(code, dot, false);
                    code.write(": ");
                    self.list_value[i + 1].write_script(code, dot, false);
                    i += 2;
                }
...
impl BlockNode {
    ...
    pub fn write_script(&self, code: &mut CodeWriter, dot: bool, sum: bool) {
        for child in &self.children {
            child.write_script(code, dot, sum);
            code.end_line();
        }
    }
}
...
impl AstNode {
    ...
    pub fn write_script(&self, code: &mut CodeWriter, dot: bool, sum: bool) {
        match self {
            ...
            AstNode::Literal(datum) => datum.write_script(code, dot, sum),
            AstNode::Block(block) => block.write_script(code, dot, sum),
            AstNode::Var(name) => code.write(name),
            AstNode::Assignment { variable, value, force_verbose } => {
                if dot && !*force_verbose {
                    variable.write_script(code, dot, sum);
                    code.write(" = ");
                    value.write_script(code, dot, sum);
                } else {
                    code.write("set ");
                    variable.write_script(code, dot, sum);
                    code.write(" to ");
            ...
            AstNode::ChunkExpr { chunk_type, first, last, string } => {
                ...
                if is_single {
                    ...
                    code.write(" ");
                    first.write_script(code, dot, sum);
                    code.write(" of ");
                    string.write_script(code, dot, sum);
                } else {
                    code.write(chunk_name);
                    code.write(" ");
                    first.write_script(code, dot, sum);
                    code.write(" to ");
                    last.write_script(code, dot, sum);
                    code.write(" of ");
                    string.write_script(code, dot, sum);
                }
            }
            AstNode::ChunkHilite(chunk) => {
                code.write("hilite ");
                chunk.write_script(code, dot, sum);
            }
            ...
            AstNode::SpriteIntersects { first, second } => {
                ...
                second.write_script(code, dot, sum);
            }
            ...
            AstNode::Member { member_type, member_id, cast_id } => {
                if dot {
                    ...
                    code.write("(");
                    member_id.write_script(code, dot, sum);
                    if let Some(cast) = cast_id {
                        code.write(", ");
                        cast.write_script(code, dot, sum);
                    }
                    code.write(")");
                } else {
                    code.write(member_type);
            ...
            AstNode::TheProp { obj, prop } => {
                ...
                code.write(prop);
                code.write(" of ");
                obj.write_script(code, dot, sum);
            }
            AstNode::ObjProp { obj, prop } => {
                if dot {
                    obj.write_script(code, true, sum);
                    code.write(".");
                    code.write(prop);
                } else {
                    code.write("the ");
            ...
            AstNode::ObjBracket { obj, prop } => {
                obj.write_script(code, dot, sum);
                code.write("[");
                prop.write_script(code, dot, sum);
                code.write("]");
            }
            AstNode::ObjPropIndex { obj, prop, index, index2 } => {
                ...
                code.write(prop);
                code.write("[");
                index.write_script(code, dot, sum);
                if let Some(i2) = index2 {
                    code.write("..");
                    i2.write_script(code, dot, sum);
                }
                code.write("]");
            }
            ...
            AstNode::SoundProp { sound_id, prop } => {
                ...
                code.write(" of sound ");
                sound_id.write_script(code, dot, sum);
            }
            AstNode::SpriteProp { sprite_id, prop } => {
                code.write("the ");
                code.write(&get_sprite_prop_name(*prop));
                code.write(" of sprite ");
                sprite_id.write_script(code, dot, sum);
            }
            AstNode::Call { name, args } => {
                write_call(code, name, args, dot, sum);
            }
            AstNode::ObjCall { name, args } => {
                if let AstNode::Literal(arg_list) = args.as_ref() {
                    if !arg_list.list_value.is_empty() {
                        ...
                        if dot {
                            ...
                            for (i, arg) in arg_list.list_value.iter().skip(1).enumerate() {
                                if i > 0 { code.write(", "); }
                                arg.write_script(code, true, sum);
                            }
                            code.write(")");
                        } else {
                            code.write(name);
                    ...
                    code.write(name);
                    code.write("(");
                    args.write_script(code, dot, sum);
                    code.write(")");
                }
            }
            ...
            AstNode::If { condition, block1, block2, has_else } => {
                code.write("if ");
                condition.write_script(code, dot, sum);
                code.write(" then");
                code.end_line();
                code.indent();
                block1.borrow().write_script(code, dot, sum);
                code.unindent();
                if *has_else && !block2.borrow().children.is_empty() {
                    code.write("else");
                    code.end_line();
                    code.indent();
                    block2.borrow().write_script(code, dot, sum);
                    code.unindent();
                }
                code.write("end if");
            }
            AstNode::RepeatWhile { condition, block, .. } => {
                ...
                code.end_line();
                code.indent();
                block.borrow().write_script(code, dot, sum);
                code.unindent();
            ...
            AstNode::RepeatWithIn { var_name, list, block, .. } => {
                ...
                code.write(var_name);
                code.write(" in ");
                list.write_script(code, dot, sum);
                code.end_line();
                code.indent();
                block.borrow().write_script(code, dot, sum);
                code.unindent();
            ...
            AstNode::RepeatWithTo { var_name, start, end, up, block, .. } => {
                ...
                code.write(var_name);
                code.write(" = ");
                start.write_script(code, dot, sum);
                if *up {
                    code.write(" to ");
                } else {
                    code.write(" down to ");
                }
                end.write_script(code, dot, sum);
                code.end_line();
                code.indent();
                block.borrow().write_script(code, dot, sum);
                code.unindent();
            ...
            AstNode::Tell { window, block } => {
                code.write("tell ");
                window.write_script(code, dot, sum);
                code.end_line();
                code.indent();
                block.borrow().write_script(code, dot, sum);
                code.unindent();
            ...
            AstNode::Case { value, first_label, otherwise, .. } => {
                ...
                while let Some(label) = current_label {
                    ...
                    label_ref.write_script(code, dot, sum);
                    current_label = label_ref.next_label.clone();
                }

                // Write otherwise
                if let Some(ow) = otherwise {
                    ow.borrow().write_script(code, dot, sum);
                }
...
impl CaseLabelNode {
    ...
    pub fn write_script(&self, code: &mut CodeWriter, dot: bool, sum: bool) {
        ...
        code.end_line();
        code.indent();
        self.block.borrow().write_script(code, dot, sum);
        code.unindent();
    }
}
...
fn write_binary_op(code: &mut CodeWriter, opcode: OpCode, left: &Rc<AstNode>, right: &Rc<AstNode>, dot: bool, sum: bool) {
    ...
    left.write_script(code, dot, sum);
    if left_needs_parens { code.write(")"); }
    ...
    right.write_script(code, dot, sum);
    if right_needs_parens { code.write(")"); }
}
